...
def create_http_app(settings: Settings) -> FastAPI:
    ...
    async def xfloor_context_middleware(request: Request, call_next):
        ...
        if not is_mcp_path:
            ...
            if path == "/openai-widget" or path.startswith("/openai-widget/"):
                logger.info(
                    "Widget asset request request_id=%s method=%s path=%s status=%s content_type=%s duration_ms=%.1f",
                    request_id,
                    method,
                    path,
                    response.status_code,
                ...
                if response.status_code >= 400:
                    logger.warning(
                        "Widget asset non-success response request_id=%s path=%s status=%s content_type=%s",
                        request_id,
                        path,
                        response.status_code,
                        content_type,
                    ...
                    logger.warning(
                        "Widget javascript asset served with unexpected content-type request_id=%s path=%s content_type=%s",
                        request_id,
                        path,
                        content_type,
                    )
