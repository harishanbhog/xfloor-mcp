...
from ..constants import WIDGET_ASSET_VERSION
...
def _is_esm_script(script_text: str | None) -> bool:
    if not script_text:
        return False
    return re.search(r"(^|\n)\s*(import|export)\b", script_text) is not None
...
def _build_query_current_floor_document(*, asset_base_url: str | None = None, preview_data: dict[str, Any] | None = None) -> str:
    ...
    cache_key = WIDGET_ASSET_VERSION.strip() or "v1"
    script_src_base = f"{base}/openai-widget/query-current-floor.js" if base else "/openai-widget/query-current-floor.js"
    style_href_base = f"{base}/openai-widget/assets/style.css" if base else "/openai-widget/assets/style.css"
    script_src = f"{script_src_base}?v={cache_key}"
    style_href = f"{style_href_base}?v={cache_key}"
